...
///
/// netstandard2.1: timeout via Task.WhenAny, file-scoped namespace,
...
public class TcpClient
{
    ...
    public async Task<ConnectResult> ConnectAsync(string ipAddress, int port, int timeout = 10000, Encoding? streamEncoding = null)
    {
        ...
        var result = await ConnectAsync(ip, port, timeout, streamEncoding ?? System.Text.Encoding.UTF8);
    ...
    public async Task<ConnectResult> ConnectAsync(IPAddress ipAddress, int port, int timeout = 10000, Encoding? streamEncoding = null)
    {
        ...
        Utilities.Logger?.Information("{Name} Trying to connect to {IpAddress}:{Port}", Name, _ipAddress, Port);
        ...
        {
            Utilities.Logger?.Warning("{Name} Connection timeout", Name);
            ConnectionFail?.Invoke(this);
            return ConnectResult.Timeout();
        }
        catch (IOException e)
        {
            Utilities.Logger?.Warning("{Name} Connection IOException: {Message}", Name, e.Message);
            Disconnect();
            ConnectionFail?.Invoke(this);
        ...
        {
            Utilities.Logger?.Error(e, "{Name} Connection error", Name);
            ConnectionFail?.Invoke(this);
        ...
        {
            Utilities.Logger?.Warning("{Name} Connection failed", Name);
            ConnectionFail?.Invoke(this);
            return ConnectResult.NotConnected();
        }
    ...
    public void Disconnect()
    {
        lock (_lock)
        {
            ...
            _stream?.Close();
            try { _reader?.Dispose(); }
            catch (InvalidOperationException e) { Utilities.Logger?.Debug("{Name} Disconnect: reader dispose skipped — async read in progress ({EMessage})", Name, e.Message); }
            try { _writer?.Dispose(); }
            catch (InvalidOperationException e) { Utilities.Logger?.Debug("{Name} Disconnect: writer dispose skipped — async write in progress ({EMessage})", Name, e.Message); }
            _tcpClient?.Close();
            _tcpClient = null;
            _stream = null;
            _reader = null;
    ...
    public async Task<ReadResult> ReadAsync(int timeout = -1)
    {
        ...
        {
            _pendingReadTask = null;
            Utilities.Logger?.Debug("{Name} ReadAsync threw: {Message} - disconnected by user?", Name, e.Message);
            return ReadResult.Fail(e);
        }
        catch (InvalidOperationException e)
        {
            _pendingReadTask = null;
            Utilities.Logger?.Debug("{Name} ReadAsync threw: InvalidOperationException {Message}", Name, e.Message);
            Disconnect();
            OnDisconnection();
            return ReadResult.NotConnected();
        }
        catch (Exception e)
        {
            ...
            Utilities.Logger?.Debug("{Name} ReadAsync threw: {Message}", Name, message);
            return ReadResult.Fail(e);
        }
    }
    ...
    {
        Utilities.Logger?.Debug("{Name} Reconnect({ShouldReconnect})", Name, ReconnectionPolicy.ShouldReconnect);
        ...
        Task.Run(async () =>
        {
            ...
            {
                Utilities.Logger?.Error(e, "{Name} Reconnection failed", Name);
            }
            cts.Dispose();
    ...
    public async Task<WriteResult> WriteAsync(string command, int timeout = -1)
    {
        ...
        {
            Utilities.Logger?.Debug("{Name} WriteAsync threw InvalidOperationException, triggering disconnection", Name);
            OnDisconnection();
        ...
        catch (Exception e)
        {
            ...
            if (e.InnerException != null)
                message = $"{message}\ninner exception:{e.InnerException.Message}\n{e.InnerException.StackTrace}";
            Utilities.Logger?.Debug("{Name} WriteAsync threw: {Message}", Name, message);
